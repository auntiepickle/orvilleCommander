...
Open the dev URL in Chrome, Edge, or Opera (WebMIDI requirement — Firefox and Safari will silently fail to connect). Grant MIDI permissions, pick input/output ports, set device ID (default 0). No unit handy (or no WebMIDI)? The "Demo Mode" button in the connection strip swaps in `src/demo.js` — a captured device tree answering through the real port contract — so the full UI is browsable offline.
...
index.html
  └─ main.js          entry point, DOM wiring, connectMidi, registerEventBridge
      ...
      ├─ bitmap.js          screen-capture denibble + canvas render (orchestration)
      ├─ framebuffer.js     pure pixel decode: denibble + parseScreenHeader + computePixels (injectable colors)
      ├─ theme.js           tokenized theme engine: presets + per-token overrides on :root, service-panel editor
      ├─ demo.js            demo mode: captured device tree (demo-data.js) served via the midi.js port contract
      ├─ demo-data.js       the canned device tree as an ES module (was demo-data.json — #137: JSON import broke Node harnesses)
      ...
      ├─ sysex-commands.js  CMD / KEY / MOD protocol constants + MOD_SOURCES table
      ├─ constants.js       named constants: STORAGE_KEY, log defaults, CACHE.STABLE_SUBTREE_PREFIXES, settle-timing ms, MIDI_MAP grid (no magic numbers)
...
Data flow: user clicks → `controls.js` sends SysEx via `midi.js` (every `OBJECTINFO`/`VALUE`/`GET_SCREEN` request joins a **dump wave** — a per-wave outstanding counter with an idle watchdog that rearms on every raw inbound packet except unsolicited `0x3c` sequence-out emits, which are not wave responses — #146; bitmap fetches defer + coalesce behind an open wave because the device drops colliding requests, and meter-poll ticks skip while a wave is open — #107) → Orville responds → `midi.js` listener (`addSysexListener` reassembles multi-packet SysEx from `F0` to `F7` before parsing — see below) → `parser.js` parses, writes state via `store.setState`, and emits events on `events.js` → `event-bridge.js` renders (C1/#37 + R7): a **progressive paint** the moment the current key's `objectinfo:received` lands, a **child-arrival repaint** when a tree-recorded child of the on-screen menu lands (so embeds appear when their data arrives, not on the next unrelated render; gang grandchildren count too via `tree.js withinGangOf` — #132), and a **settled paint** when the wave ends (`dumpComplete` — drained, or the idle watchdog fired on a stall); `hideLoading` fires only on waves that carried OBJECTINFO or GET_SCREEN requests (#3) or on a watchdog stall, so value-only meter-poll waves cannot clear an unrelated loading state. There are no render timers or debounce; a render that issues new requests (missing values) opens a new wave whose drain triggers the next settled paint — the parser's all-COL fan-out covers embed prefetching, and descends one level further into **gang COL subtrees** (#132: blank-tag, non-`'0'`-position COLs — the device's ganged-parameter pages, e.g. the I/O routing matrix — which `renderer.js renderGangInline` inlines as one editable page instead of softkeys; see `docs/device-model.md` §3 "Gang COL subtrees"). One render guard (#131): while a SET dropdown inside `#lcd` is focused, repaints are **deferred** — the latest paint parks and replays on blur, or is discarded on change (the post-change refresh supersedes it) — so progressive paints cannot destroy an open select mid-interaction. Connect/navigation one-shots live in the bridge too (C2/#38): `selectPorts` resets the view to root and arms `pendingLanding`; the root dump's arrival lands on the active preset, and a `pendingDescend` one-shot (also set by nav clicks) descends once into a COL-only menu's first child when that menu's dump arrives — there is no timer and no sticky `autoLoad` flag. The landing also arms the **eager loader** (#106, gated by `appState.eagerLoad`): on the first clean wave drain after it (`all-received` — watchdog stalls keep the arm, since the landing wave routinely stalls on the bitmap transfer, R5a), `eager-loader.js` walks the active preset's COL subtree breadth-first with exactly ONE OBJECTINFO in flight, skipping tree-cached nodes, so structure is warm for the R3 pre-paint without ever duplicating the parser's fan-out. Bitmap path: `0x17` SysEx → `parser.js` denibbles (`bitmap.js`) → emits `screen:received` → `event-bridge.js` → `renderBitmap` on the canvas — lit/unlit pixels render in the active theme's `--lcd-px`/`--lcd-bg` tokens (`framebuffer.js:computePixels` takes injectable on/off colors, defaulting to the historical green-on-black), and a theme change recolors the last frame immediately via `bitmap.js:rerenderBitmap` without a new fetch.

Parameter editing is in-glass: clicking a NUM/STR value swaps the span for an `.lcd-edit` input inside `#lcd` (Enter validates + commits, Escape cancels, invalid input flashes and stays for correction). There are NO browser `prompt()`/`alert()` boxes. A focused inline editor parks repaints exactly like an open SET dropdown (the #131 defer guard covers both).
...
| `0x3c` | in | SEQUENCE_OUT — unsolicited changed-field key+value when `sequence out = new` (MIDI mapping); not a wave response, does not rearm the watchdog |
...
### Demo Mode dataset (`src/demo-data.js`)

The canned device tree `src/demo.js` serves. (Shipped as an ES module since #137 — the earlier `.json` import broke every Node harness.) Provenance: the shipped capture was mined from session logs + fixtures (its `capturedAt` field says so); device ID 1, ~98 OBJECTINFO nodes to depth 4 plus one real `0x17` screen frame. The refresh script (`logs/generate-demo-data.mjs`, named in `demo-data.js`'s header) lives under gitignored `logs/`, so the recipe is recorded here: against the powered Orville via `@julusian/midi` (close any app tab first — WinMM is single-client), BFS the object tree from the root key with raw sequential OBJECTINFO_DUMP requests, exactly ONE in flight (the tree-audit phase-1 recipe), descending COL children to depth 5 with ~100 ms between requests and `F0`..`F7` frame reassembly on the input; then fetch one screen frame (`0x18`). Write `{ capturedAt, deviceId, objectinfo: { key: asciiDump }, screenFrame }` as an ES module (`export default {…}`) to `src/demo-data.js` — `demo.js` derives everything else (values, SET option tables) from the dumps.

## Where state lives

Almost all runtime state is on `appState` (state.js). The per-menu cache `currentValues` is cleared in exactly one place — `renderer.js:updateScreen`, which every navigation path funnels through — so don't add per-handler clears (C8/#44). Device STRUCTURE lives in `src/tree.js` (T1b/#105): the parser tree-records every OBJECTINFO dump (`recordDump`), and child lookups (`getNode`, `findParamUnder`), labels (`labelForSub` — every COL child gets an affordance, blank nodes get a child-derived or `...` label), and ancestry all read the tree; the old per-menu `childSubs` cache is deleted. Freshness is per-visit child refetch EXCEPT in stable subtrees (#113, `constants.CACHE.STABLE_SUBTREE_PREFIXES` — the program subtree): there the fan-out skips the STRUCTURE refetch for `tree.js isFresh()` children (param values still refresh every visit). Staleness is per-key: any `sendValuePut` into the subtree, any `sendKeypress`, or Sync/reconnect stales the cached keys, and a key is fresh again only when its own dump re-records. `keyStack` is DERIVED state: every navigation sets it via `tree.js:deriveKeyStack` (tree ancestry, not click history; one deliberate exception — the static bottom-row root jump RESETS it to `[]`, R2), and entries keep the canonical `{key, tag, subs}` shape — never raw strings (C3/#39). The tree is module state like `logger.js`, which owns its own log level and per-category visibility as private module state (defaults from `constants.DEFAULT_LOG_CATEGORIES`), set via `setLogLevel` / `setLogCategories`. This was moved off `appState` (C6) so `logger.js` no longer imports `state.js`, breaking the `store` → `logger` → `state` → `store` import cycle. Everything persistent is in `localStorage.midiConfig` (config.js). There is no other store. If you need durable state, add it to `midiConfig` — either through `saveConfig` (the Save Config button's positional full save, which merge-preserves keys it does not own so it cannot drop other writers' data) or through a dedicated single-key saver like `saveThemeConfig`, which the theme editor calls on every preset/swatch change, independent of the Save Config button. Two more single-key savers follow the same self-merging read-modify-write pattern: `saveLibraryConfig` persists the synced preset library (`presetLibrary` — #142, survives until re-sync) and `saveMidiMappings`/`loadMidiMappings` persist the program-scoped MIDI-map badges (`midiMappings` — #146; `midi-map.js` loads them at module init and re-saves on every record/clear, so the mapped-param badges survive a page reload, HMR included). The full `midiConfig` key set: `outputId`, `inputId`, `deviceId`, `logLevel`, `logCategories`, `fetchBitmap`, `updateBitmapOnChange`, `presetKey`, `eagerLoad` (the Save Config positional save), plus `theme`, `presetLibrary`, and `midiMappings` (each owned by its own single-key saver).

Lifecycle signals ride the `events.js` bus: `library.js` emits `program:loaded` ({dspSlot}) when a program finishes loading to a DSP. It is an emitted signal with **no active subscriber** today — the MIDI-map badges deliberately do NOT clear on it (they are program-scoped and persisted, so loading a different program simply shows that program's badges; see `tests/midi-map.test.js`).
...
1. `npm test` passes.
2. `npm run dev` — connect to real Orville (or, if offline, click "Demo Mode" and confirm the captured tree lands and navigates, or upload a known-good debug file via the "Process Debug File" button and confirm the bitmap renders).
3. Navigate into a preset, change a SET parameter, confirm the LCD updates and the device reflects the change. Click a NUM value and confirm the in-glass editor appears (Enter commits, Escape cancels) — no browser prompt boxes.
4. Toggle A/B; confirm the clicked preset name highlights and both DSPs still render.
5. In the service panel's THEME column, switch presets — the faceplate, LCD, and bitmap canvas all retint, and the choice survives a reload (persisted in `midiConfig`).
6. Open the preset browser (Browse): run a name search, preview a program on the idle DSP and confirm the previous program is restored when you dismiss the preview, then load to A and B. Run a library Sync and confirm the LCD defrag-style progress overlay (`sync-dialog.js`) draws and the bank map fills.
7. Open the MIDI Map panel: map a parameter to a controller (pick a source / range / type, or Learn), confirm the mapped badge lights on that param in the LCD, and confirm the badge survives a page reload (persisted to `midiConfig.midiMappings`). On a multi-block program (e.g. Horrors), confirm a sub-block param (e.g. reverb `lowfreq`) binds to the right parameter.
