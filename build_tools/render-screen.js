...
//
// Usage:
//   node build_tools/render-screen.js <fixture.txt> <out.png> [scale=6] [shiftMode=wrap]
//
...
// shiftMode is wrap | black | none (see framebuffer.js computePixels).
...
// ----- main ----------------------------------------------------------------
const [inFile, outFile, scaleArg, shiftMode = 'wrap'] = process.argv.slice(2);
if (!inFile || !outFile) {
  console.error(
    'Usage: node build_tools/render-screen.js <fixture.txt> <out.png> [scale=6] [shiftMode=wrap|black|none]'
  );
  process.exit(1);
}
...
const pixels = computePixels(rawBytes, { width: SCREEN_W, height: SCREEN_H, shiftMode });
...
console.log(`Wrote ${outFile} (${scaled.width}x${scaled.height}, shiftMode=${shiftMode})`);
