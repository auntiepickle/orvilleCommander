...
Step 5's `setState(partial, origin)` was introduced to establish an audit tripwire for state mutations — the `[stateWrite]` trace category records every call's origin and key-set. In practice, only `parser.js` honors this path. All other direct `appState.x = y` writes across the codebase bypass the audit: `renderer.js`'s autoload branch writes `keyStack`, `currentKey`, `autoLoad` directly; `renderer.js`'s `updateScreen` clears `childSubs`, `currentValues`, and sometimes `currentSoftkeys` directly; `renderer.js`'s `handleLcdClick` mutates several fields across its branches directly; `midi.js:24`'s `setMidiPorts` writes `appState.deviceId` directly; `main.js` writes `pollingEnabled` and `logLevel` directly at various user-interaction points; and even `parser.js` itself has non-migrated writes at `:132` (`currentValues`), `:120` (favorites-fix `currentValues`), and `:99` (`childSubs`). The effect is that `[stateWrite]` covers only a small fraction of actual state mutation — the audit tripwire is partial, and any debugging session assumes more coverage than it has.

**Implication for Step 7.** The events-bus migration is the natural completion point for the audit. Every direct write in the codebase is a candidate for migration to `setState(patch, origin)` as part of Step 7's rewiring — not just the writes inside parser.js. The Step 5.5 startup test's Tier A sequence catches parser-origin writes only; un-migrated writes remain invisible to the characterization test, which means Step 7 can remove or rewire them without the test objecting. Reviewers of Step 7 diffs should cross-check against the inventory above rather than relying on the test alone.
