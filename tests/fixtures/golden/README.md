...
These are **complete** captures: each `.txt` is the full 1933-byte denibbled
screen dump (12-byte header + 1920 pixel bytes + checksum) and passes the
`parseScreenHeader` integrity check (`complete && checksumOk`). The capture tool
reassembles the multi-buffer SysEx the device sends over its slow DIN link (see
tracker FB5).
